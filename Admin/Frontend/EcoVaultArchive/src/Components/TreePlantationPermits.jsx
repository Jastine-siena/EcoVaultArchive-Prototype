...
const TreePlantationPermits = () => {
  const [file, setFile] = useState(null);
  const [documents, setDocuments] = useState([]);
  const [filteredDocuments, setFilteredDocuments] = useState([]);
  const [dropdownOpen, setDropdownOpen] = useState(null); // Manage dropdown state
  const [searchTerm, setSearchTerm] = useState('');
  const sourceTable = 'tpr'; // Tree Plantation Permits table
  const destinationTables = ['treecp', 'lt', 'tp', 'cr']; // List of destination tables
  ...
  const handleMove = async (filename, destinationTable) => {
    try {
      ...
      // Refresh documents list after move
      const updatedDocuments = documents.filter(doc => doc !== filename);
      setDocuments(updatedDocuments);
      setFilteredDocuments(updatedDocuments);
      setDropdownOpen(null); // Close dropdown after move
    } catch (error) {
  ...
  return (
    <div className="container mt-4">
      <h2 className="text-success mb-4 text-dark">Tree Plantation Permits Document Lists</h2>
      <div className="container mt-4">
...
export default TreePlantationPermits;
